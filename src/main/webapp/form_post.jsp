    ...
    <div class="row">
      ...
      <form action="${pageContext.request.contextPath}/post/save" method="post" class="col-8">
        ...
        <div class="mb-3">
          ...
          <textarea id="post_content" name="post_content" class="form-control" rows="4" required><c:out value="${post.content}" /></textarea>
        </div>
        ...
        <div class="mb-3">
          ...
          <c:choose>
            <c:when test="${not empty post.postDate}">
              ...
              <input type="date" id="post_date" name="post_date" class="form-control" value="${dt}" required />
            </c:when>
            <c:otherwise>
        ...
        <div class="mb-3">
          ...
          <select id="postuser_id" name="postuser_id" class="form-select" required>
            <option value="">-- selecione --</option>
            <c:forEach var="u" items="${users}">
              <option value="${u.id}" <c:if test="${post.user != null and u.id == post.user.id}">selected</c:if>>
                <c:out value="${u.name}" />
              </option>
            </c:forEach>
          </select>
